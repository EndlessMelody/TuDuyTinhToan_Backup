...
// We will build DashboardLayout.tsx
let dlContent = `"use client";
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Footer info at the bottom of the left sidebar */}
        {isSidebarOpen && (
          <Row horizontal="between" paddingTop="l" borderTop="neutral-alpha-weak">
            <Text variant="body-default-xs" onBackground="neutral-weak">TasteMap v1.0.0-beta</Text>
            <Row gap="16">
              <Text variant="body-default-xs" onBackground="neutral-weak" style={{ cursor: "pointer" }}>Feedback</Text>
              <Text variant="body-default-xs" onBackground="neutral-weak" style={{ cursor: "pointer" }}>Terms</Text>
            </Row>
          </Row>
        )}
      </Column>
      ...
      <Column flex={1} fillHeight overflowY="auto" padding="xl">
        {children}
      </Column>
...
fs.writeFileSync(path.join(__dirname, 'src', 'components', 'DashboardLayout.tsx'), dlContent);
console.log('DashboardLayout.tsx created.');

// Now strip page.tsx of the Left Sidebar, Right Sidebar, Footer, and update the container.

// We will find the boundaries.
let leftSidebarStart = content.indexOf('{/* ═══════════ 1. LEFT SIDEBAR ═══════════ */}');
let centerStart = content.indexOf('{/* ═══════════ 2. CENTER PANEL ═══════════ */}');
let rightSidebarStart = content.indexOf('{/* ═══════════ 3. RIGHT SIDEBAR (Social Tracking Panel) ═══════════ */}');

// Delete radarData array, SidebarItem function
let p0 = content.indexOf('const radarData = [');
let p1 = content.indexOf('function getDynamicContext');
if (p0 > 0 && p1 > 0) content = content.substring(0, p0) + content.substring(p1);

p0 = content.indexOf('function SidebarItem(');
p1 = content.indexOf('// ═════════ LOBBY CARD ═════════ //');
if (p0 > 0 && p1 > 0) content = content.substring(0, p0) + content.substring(p1);

// Inside DashboardPage, delete `isSidebarOpen`, `isRightExpanded`, `isCreateRoomOpen`, `handleComingSoon` (optional, but keep it just in case if button uses it)
...
// The root return statement in DashboardPage currently wraps everything in:
// return (
//   <Row fillWidth background="page" overflow="hidden" style={{ height: "100vh", color: "#1C1C1E" }}>
//     <Column LeftSidebar>...</Column>
//     <Column CenterPanel>...</Column>
//     MODALS
//     <Column RightSidebar>...</Column>
//   </Row>
// )

// We will replace the entire return block manually using regex or split. Let's just do a string replacement for the big chunks.

// Delete Footer:
// {/* ═══ MINI STATUS BAR (Dashboard Footer) ═══ */}
// ... up to MODALS
let p2 = content.indexOf('{/* ═══ MINI STATUS BAR (Dashboard Footer) ═══ */}');
let p3 = content.indexOf('{/* ═══════════ MODALS ═══════════ */}');
if (p2 > 0 && p3 > p2) {
  content = content.substring(0, p2) + content.substring(p3);
}

// Write the modified file to disc, and then we will clean up the wrapping tags manually using replace_file_content or a script.

fs.writeFileSync(path.join(__dirname, 'src', 'app', 'page.tsx'), content);
console.log('page.tsx partially stripped.');
